...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    cb(null, `building_${req.body.buildingId}_floor_${req.body.floorNumber}_${Date.now()}${path.extname(file.originalname)}`);
  }
});
...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (mimetype && extname) {
      ...
      cb(new Error('Только изображения (jpeg, jpg, png) допустимы для загрузки!'));
    }
  }
}).single('planImage');
