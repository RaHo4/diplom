...
const UsersList = ({ navigation }) => {
  ...
  const handleRejectPendingUser = async (userId) => {
    try {
      ...
      await api.delete(`/users/pending/${userId}`);
  ...
  const getRoleName = (role) => {
    switch (role) {
      ...
      case 'duty': return 'Дежурный по зданию';
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.userInfo}>
              <View style={[
                styles.userAvatar,
                { backgroundColor: item.role === 'admin' ? '#d32f2f' :
                                 item.role === 'duty' ? '#ff9800' :
                                 item.role === 'dispatcher' ? '#2196f3' : '#4caf50' }
              ]}>
              ...
              <ScrollView>
                ...
                <View style={styles.formGroup}>
                  ...
                  <View style={styles.roleSelector}>
                    {['admin', 'duty', 'dispatcher', 'firefighter'].map(role => (
                      <TouchableOpacity
                        key={role}
                        style={[
                          styles.roleOption,
              ...
              renderItem={({ item }) => (
                <View style={styles.pendingUserCard}>
                  <View style={styles.pendingUserInfo}>
                    <Text style={styles.pendingUserName}>{item.fullName}</Text>
                    <Text style={styles.pendingUserEmail}>{item.email}</Text>
                    <Text style={styles.pendingUserRole}>{getRoleName(item.role)}</Text>
                  ...
                  <View style={styles.pendingActions}>
                    <TouchableOpacity 
                      ...
                      onPress={() => handleRejectPendingUser(item.id)}
                    >
                    ...
                    <TouchableOpacity 
                      ...
                      onPress={() => handleApprovePendingUser(item.id)}
                    >
